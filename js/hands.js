...
AFRAME.registerComponent('hand-rig', {
  ...
  init: function () {
    ...
    this.openness = 0;
    this.pinch = 0;
    this.pointing = 0;          // v8: 1 while this hand holds a point gesture
    this.pointSpread = 0;       // the raw measure behind it, for tuning
    this.holding = 0;           // v9.3: 1 while this hand is closed and held
    this.holdT = 0;             // how long it has been closed, in seconds
  ...
  //  v9.3: `dtMs` is read now -- the hold gesture below needs a dwell
  //  timer, and this component had never had a use for the clock before.
  //  A-Frame passes it either way; nothing else here changed.
  tick: function (time, dtMs) {
    ...
    if (!this.readDirect() && !this.readFromComponent(c)) {
      ...
      //  a hand that has gone must not leave a point gesture latched --
      //  the swarm would keep following an aim point that no longer has
      //  a hand behind it
      this.pointing = 0;
      //  ...and the same for the hold, or a butterfly would keep circling
      //  a hand that has left the tracking volume
      this.holding = 0; this.holdT = 0;
      return;
    }
    var p = this.poses;
    this.tracked = true;
    ...
    //  v8: IS THIS HAND POINTING? Index out, the other three curled.
    //
    //  Nothing in the piece could ask this before. The ray is cast for
    //  any tracked hand regardless of pose, which is fine for selection
    //  -- you reach at a butterfly and it lights up -- but useless as a
    //  trigger, because "hand is visible" is true almost always.
    //  `openness` above cannot answer it either: a pointing hand has one
    //  finger out and three in, so it averages to the middle of the
    //  range and is indistinguishable from a half-open hand.
    //
    //  Costs nothing -- the same three fingertips were just measured for
    //  openness, so this is a second reading of numbers already in hand.
    //
    //  The GAP between the index and the curled three is the gesture,
    //  not the index's own length: an absolute length would read a
    //  splayed palm (every finger long) as a point.
    //
    //  HYSTERESIS, for the same reason the pinch has two thresholds and
    //  not one -- a single boundary chatters, and a swarm flickering
    //  between following and released reads as broken rather than alive.
    var others = 0;
    for (i = 1; i < tips.length; i++) {          // skip the index, tips[0]
      others += this.jointPos(p, tips[i], this._v).distanceTo(this.wrist);
    }
    others /= (tips.length - 1);
    this.pointSpread = this.indexTip.distanceTo(this.wrist) - others;
    var gate = this.pointing > 0.5 ? CFG.pointOff : CFG.pointOn;
    this.pointing = this.pointSpread > gate ? 1 : 0;

    //  v9.3: IS THIS HAND BEING HELD CLOSED? The gesture the `holding`
    //  icon shows -- a closed hand, offered and kept there.
    //
    //  `openness` already measures it and has since v2 with nothing
    //  reading it for this: 0 is a fist, 1 is a splayed palm. Two
    //  thresholds, like the pinch and the point, or it chatters.
    //
    //  THE DWELL IS THE PART THAT MATTERS, and neither of the other two
    //  gestures needs one. Catching a letter closes the hand as well --
    //  a pinch IS a partial fist -- so without a dwell every single
    //  capture would read as the beginning of a hold and the butterfly
    //  you just caught would be chased by the one you made earlier. A
    //  hold is something you keep doing; requiring half a second of it
    //  separates the two cleanly without feeling like a wait.
    //
    //  A pointing hand can never also be holding: three curled fingers
    //  drag `openness` down far enough to cross the threshold, and the
    //  two gestures would fight over the same butterflies.
    var closed = this.openness < (this.holding > 0.5 ? CFG.holdOff : CFG.holdOn);
    if (closed && !this.pointing) {
      this.holdT += dt;
      if (this.holdT >= CFG.holdDwell) { this.holding = 1; }
    } else {
      this.holdT = 0;
      this.holding = 0;
    }
