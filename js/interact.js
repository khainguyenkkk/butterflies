...
AFRAME.registerComponent('pointer-input', {
  init: function () {
    ...
    this.foci = [];
    //  v9.3: and the same for HELD hands -- a list of hand positions a
    //  born butterfly can come and settle on. Rebuilt every tick for the
    //  same reason `foci` is: a hand that opens, or leaves the tracking
    //  volume, drops out by simply not being re-added.
    this.holds = [];
  ...
  tick: function (time, delta) {
    ...
    //  v9.3: THE GREETING HOLDS THE ROOM. While the welcome voice is
    //  speaking, this component does nothing at all -- no picking, no
    //  hover, no pinch, no aim points and no hold points. A visitor who
    //  has just put the headset on has their hands up somewhere, and a
    //  stray pinch during the welcome means they start their turn with
    //  letters already caught, before they have been told what any of it
    //  does.
    //
    //  Returning HERE rather than guarding each gesture is deliberate:
    //  it is one branch, it cannot be partially applied, and the lists
    //  below are cleared first so nothing stays latched from the frame
    //  before the lock came on.
    if (typeof Welcome !== 'undefined' && Welcome.locked()) {
      this.foci.length = 0;
      this.holds.length = 0;
      kb.setHot({});
      for (var q = 0; q < this.lines.length; q++) {
        if (this.lines[q]) { this.lines[q].visible = false; }
      }
      //  drop any latched pinch/click, or the frame the lock lifts would
      //  fire whatever was pressed during it
      for (var r = 0; r < this.pointers.length; r++) {
        this.pointers[r].closed = false;
        this.pointers[r].click = false;
        this.pointers[r].hover = null;
      }
      return;
    }
    ...
    for (var i = 0; i < this.pointers.length; i++) {
      ...
      if (p.kind === 'hand') {
        ...
        //  v9.3: A HELD HAND. Published as a POSITION, not a ray -- this
        //  gesture is not about aiming somewhere, it is about the hand
        //  itself being a place to land. The wrist rather than the
        //  fingertip: a closed hand's fingertips are curled into the palm
        //  and wander as the fist tightens, while the wrist is the
        //  steadiest joint on a closed hand and sits about where anyone
        //  would say their hand "is".
        //
        //  Same finite guard as pushFocus, and for the same reason: one
        //  NaN propagates into every orbit centre and the born swarm is
        //  gone for the rest of the session, silently.
        if (rig.holding > 0.5 && isFinite(rig.wrist.x) &&
            isFinite(rig.wrist.y) && isFinite(rig.wrist.z)) {
          this.holds.push({ pos: rig.wrist });
        }
