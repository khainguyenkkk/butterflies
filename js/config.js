...
var CFG = {
  ...
  //  THE GREETING HOLDS THE ROOM. For as long as the voice is speaking,
  //  every butterfly interaction is switched off: no picking, no pinch,
  //  no pointing, no holding, no wave, and no letter-key shortcuts.
  //
  //  This is not politeness, it is how the piece is actually used. A
  //  visitor who has just put the headset on has their hands up
  //  somewhere, and any of those hands landing on a butterfly during the
  //  welcome means they start their turn with letters already caught, or
  //  the swarm already dragged out of shape, before they have been told
  //  what any of it does. The greeting is the one moment the piece is
  //  talking rather than listening.
  //
  //  Timed rather than driven off the audio finishing. The voice is
  //  SCHEDULED immediately but stays silent until the AudioContext is
  //  resumed by a user gesture (see audio.js) -- so "the clip ended"
  //  can arrive far later than the text did, or never on a machine where
  //  the gesture never comes. A clock always ends. Slightly longer than
  //  welcomeSpeechSec so the lock outlasts the last word rather than
  //  releasing under it.
  welcomeLockSec:     17.0,
  ...
  //  These position the card's outer EDGE, not its centre -- tutorial.js
  //  insets by half the card, so the whole thing stays on screen at any
  //  aspect. 1.0 would put that edge exactly on the viewport boundary.
  tutorialDockDist:   1.00,
  tutorialDockFracX:  0.94,  // of the half-width, toward the right edge
  tutorialDockFracY:  0.94,  // of the half-height, toward the top
  ...
  // ---- holding: a closed hand, held (v9.3) ----
  //  The gesture the `holding` icon shows: a closed hand, offered and
  //  kept there. `openness` (0 = fist, 1 = splayed) already measures it,
  //  and has since v2 without anything reading it for this.
  //
  //  Two thresholds, like the pinch and the point, or it chatters.
  holdOn:      0.30,        // openness BELOW this starts a hold
  holdOff:     0.45,        // ...and above this it releases
  //  A DWELL, which the other two gestures do not have and this one
  //  needs. A pinch closes the hand too, briefly -- catching a letter
  //  would otherwise read as the start of a hold every single time. A
  //  hold is a thing you keep doing; half a second of it is enough to
  //  separate the two without feeling like a wait.
  holdDwell:   0.55,        // seconds closed before it counts
  ...
  //  true or it is not.
  // ---- what a HELD hand does to the born butterflies (v9.3) ----
  //  Point gathers the whole group loosely toward a spot across the room.
  //  Holding is the opposite in every respect: it is one hand, close to
  //  the body, and what comes to it comes all the way and STAYS, circling
  //  slowly like something that has settled on you rather than something
  //  being herded.
  //
  //  It takes precedence over pointing when both are somehow true, and it
  //  reuses the same orbit-centre machinery -- which is why "the
  //  butterfly follows my hand" falls out of the flight that already
  //  exists rather than needing a second motion model. Only the constants
  //  differ, and they differ a lot.
  holdRadius:      0.30,   // metres it circles the hand at. Close.
  holdReachRadius: 6.0,    // how far away a butterfly still answers a hand
  holdMaxPull:     1.0,    // FULL commitment, unlike pointing's 0.55 --
                            // a butterfly on your hand is on your hand
  holdEase:        0.55,   // per second; slow, per the brief
  holdOrbitRate:   0.28,   // radians/sec circling the hand -- a hover
  holdReleaseTau:  2.60,   // seconds to let go and drift back out
  //  THE OVERSHOOT, and it is deliberately looser than pointing's. A
  //  butterfly arriving at a hand should sail slightly past and curve
  //  back, the way something alive does when it lands -- a spring that
  //  merely approaches reads as a cursor snapping to a target.
  holdSpringK:     3.4,    // softer than pointSpringK (9.0): slower approach
  holdSpringDamp:  0.42,   // well under 1 -- a real, visible overshoot
  holdCentreMaxSpeed: 0.75, // m/s ceiling on the centre, as everywhere else
