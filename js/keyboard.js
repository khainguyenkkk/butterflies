...
AFRAME.registerComponent('butterfly-keyboard', {
  init: function () {
    ...
    this._wv = new THREE.Vector3();        // v9.2: the wave force, per key
    ...
    //  v9.1: WAIT FOR THE TYPEFACE. Every letter in the swarm is a glyph
    //  drawn to a canvas exactly once, cached, and never redrawn -- so
    //  building before Kavoon has loaded bakes the fallback face into all
    //  twenty-six of them permanently, with no error anywhere. Type.ready
    //  fires immediately once the font is in, and has its own timeout so
    //  this can never hang the piece on a bad network. See js/type.js.
    Type.ready(function () {
      self.buildKeys();
      self.buildPanel();
    ...
    window.addEventListener('keydown', function (e) {
      ...
      if (self.done) { return; }
      //  v9.3: silent while the greeting is speaking, like every other
      //  way into the swarm. `W` is the desktop trigger for the welcome
      //  itself (welcome.js), and without this it would immediately catch
      //  a letter W on the way past.
      if (typeof Welcome !== 'undefined' && Welcome.locked()) { return; }
  ...
  buildPanel: function () {
    ...
    for (var i = 0; i < specs.length; i++) {
      ...
      //  v8: THE MARK. Parented INTO the flower's own group, which is
      //  the whole trick -- tickUI() applies every per-frame transform
      //  (the drift, the sway, the cant/tilt, the press spring) to that
      //  group, so a child inherits all of it and needs no tick code at
      //  all. b.ui.shape() only walks the six lobes it captured in its
      //  closure, so a seventh child is left alone.
      //
      //  z clears the lobes: they sit at i * 0.0006, so the topmost is
      //  at 0.003 and anything past that draws in front of the petals.
      var iconSize = CFG.blobW * sp.k * CFG.ctlIconScale;
      var icon = UI.iconPlate(sp.id, iconSize, iconSize, CFG.bg);
      icon.position.z = 0.005;
      b.mesh.add(icon);

      this.root.add(b.mesh);
      this.buttons.push({
        id: sp.id, ui: b, hue: sp.hue, lit: sp.lit, icon: icon,
  ...
  activate: function (id, handPos) {
    ...
    //  v9: the red control finally has a sound of its own, the counterpart
    //  to the green one's winner. Fired HERE rather than inside backspace()
    //  so it is the press that is audible -- and in practice the two are
    //  the same, because targets() only offers the controls once something
    //  has been typed, so this cannot fire on a press with nothing to take.
    if (id === 'delete') { this.bump('delete'); SFX.playRedButton(); this.backspace(); return; }
    for (var i = 0; i < this.keys.length; i++) {
      var k = this.keys[i];
  ...
  accept: function () {
    ...
    //  v9.1: the second prompt. Nothing in the scene says that an
    //  extended index finger gathers the butterflies you have made -- a
    //  visitor could finish a whole session without discovering the
    //  gesture v8 added. Fired here rather than from app.js's listener
    //  because it is about the CONTROL that was just pressed, not about
    //  the name meaning anything.
    //  ...after a pause. Pressing the green control is the loudest moment
    //  in the piece and a prompt on top of it is one thing too many to
    //  read; the delay also lands the card as the newborn butterfly
    //  finishes presenting itself. See CFG.tutorialPointDelaySec.
    if (typeof Tutorial !== 'undefined') {
      Tutorial.show('point', CFG.tutorialPointDelaySec);
    }
  ...
  tickUI: function (t, dt) {
    var i;
    //  v9.2: the typed name and the two flowers stand down with the rest
    //  of the room while a newborn presents itself. The ONLY things that
    //  keep their brightness are the butterfly and the label carrying its
    //  name -- both of which live in born.js and are untouched by this.
    var dim = this.dimFactor();
    ...
    for (i = 0; i < this.buttons.length; i++) {
      ...
      //  ...and the flower stands down with everything else. setDim
      //  re-derives from the remembered state colour rather than writing
      //  over it, so refreshPanel() and the dim cannot fight (see
      //  ui.js:blob).
      if (b.ui.setDim) { b.ui.setDim(dim); }
    }

    //  ...and the sky itself, which is what "the background" most
    //  literally means on desktop. In an AR session the sky is hidden and
    //  the real room shows through, so this does nothing there -- there is
    //  no way to dim a passthrough feed, and the swarm/UI dimming above is
    //  what carries the effect in the headset.
    if (this._sky === undefined) {
      var sk = document.querySelector('a-sky');
      this._sky = (sk && sk.getObject3D('mesh')) || null;
      if (this._sky) { this._skyBase = this._sky.material.color.clone(); }
    }
    if (this._sky && this._skyBase) {
      var d = 1 - (1 - dim) * CFG.bornDimSky;
      this._sky.material.color.setRGB(this._skyBase.r * d,
                                      this._skyBase.g * d,
                                      this._skyBase.b * d);
    }
  },
  ...
  readSources: function (dt) {
    ...
    for (var i = 0; i < this.sources.length; i++) {
      ...
      //  v9.2: the velocity VECTOR, not just its magnitude. The existing
      //  scatter only ever needed a speed because it pushes radially away
      //  from the hand; a wave needs to know which way the hand went.
      if (!s.vel) { s.vel = new THREE.Vector3(); }
      s.vel.copy(s.pos).sub(s.prev).divideScalar(Math.max(dt, 1e-4));
      s.prev.copy(s.pos);
      this.repulsors.push({ pos: s.pos, speed: s.isCamera ? s.speed * 0.7 : s.speed });
    }
    //  ...and hand the same sources to the wave field, which picks out the
    //  ones moving fast enough to count as a sweep. Costs nothing when
    //  nobody is waving: it rebuilds an empty list.
    Wave.update(this.sources, dt);
  },
  ...
  tickKey: function (k, t, dt) {
    ...
    //  v9.2: THE WAVE. A separate force from the scatter above and a
    //  different gesture: scatter pushes away from a fast hand, this
    //  pushes ALONG the sweep. Localised to CFG.waveRadius, so it
    //  disturbs the part of the flock the hand passes through and leaves
    //  the rest exactly as it was.
    //
    //  It only ever adds velocity. The damping and the spring back to the
    //  path immediately below are what return everything to normal once
    //  the waving stops, which is why there is no release code for this
    //  anywhere.
    this._wv.set(0, 0, 0);
    if (Wave.forceAt(k.pos, this._wv)) {
      k.offsetVel.addScaledVector(this._wv, dt);
    }
  ...
  //  v9.2: how far the room stands down while a newborn is presenting.
  //  Read off Born rather than pushed in, so this file still knows nothing
  //  about butterflies being born -- it just knows the scene has a dim.
  //  `typeof` guarded because born.js is a later <script> and could be
  //  absent from a cut-down build.
  dimFactor: function () {
    var d = (typeof Born !== 'undefined' && Born.dim) || 0;
    return 1 - d * CFG.bornDimAmount;
  },
  ...
  dress: function (k, scaleMul, alpha) {
    ...
    //  The dim multiplies what is DRAWN, never k.alpha itself -- folding
    //  it into the eased state would make the swarm fade back in from
    //  wherever the dim left it, and the capture animations read alpha to
    //  decide what they are doing.
    k.bm.setOpacity(k.alpha * this.dimFactor());
    ...
    var lp = k.letter.place(this._camPos, k.pos, k.size * k.scale,
                            k.alpha * this.dimFactor(), this._clock);
  ...
  remove: function () {
    ...
    for (var j = 0; j < this.buttons.length; j++) {
      this.buttons[j].ui.dispose();
      //  blob.dispose() only knows about its six lobes and their shared
      //  material -- the mark is a child it never saw, so it leaks
      //  unless it goes here too.
      if (this.buttons[j].icon) { this.buttons[j].icon.dispose(); }
    }
  }
});
