...
AFRAME.registerComponent('tutorial-cards', {
  init: function () {
    ...
    this.CARDS = {
      ...
      wave:  { text: 'Waving to see their harmony.',
               icon: 'icons/waving.png', next: 'hold' },
      hold:  { text: 'Holding hand to attract your butterfly.',
  ...
  dockOffset: function (cam, out, slot) {
    ...
    //  the card's own size at docked scale, in metres at dockDist
    var cardW = CFG.tutorialDockWidth;
    var cardH = cardW * (CFG.tutorialTexH / CFG.tutorialTexW);
    var step = cardH * (1 + CFG.tutorialStackGap);

    //  ANCHORED BY ITS EDGES, NOT ITS CENTRE, and the difference is not
    //  cosmetic. A mesh is positioned by its centre, so putting that
    //  centre at 70% of the half-width leaves the outer HALF-CARD hanging
    //  past it -- fine on a wide view, and clipped straight off the screen
    //  on a narrow one. Found exactly that way in a narrow preview pane,
    //  and a Quest's aspect is not the desktop's either.
    //
    //  So the fraction positions the card's outer EDGE, and the centre is
    //  then inset by half the card. Whatever the fov or aspect, the whole
    //  card is on screen with a consistent margin.
    var x = halfW * CFG.tutorialDockFracX - cardW * 0.5;
    var y = halfH * CFG.tutorialDockFracY - cardH * 0.5 - slot * step;
    return out.set(x, y, -dist);
  },
