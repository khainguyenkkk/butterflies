...
var BflyModel = (function () {
  ...
  //
  //  build(wingAlpha, cssColour, letterAlpha, letterColour, opts)
  //
  //  v7 adds the optional trailing `opts`:
  //
  //    opts.wingMap   a COLOUR texture for the wing, instead of the flat
  //                   fill every other butterfly in the piece uses.
  //
  //  Only the born butterfly passes it (see js/born.js and
  //  js/wing-paint.js) -- the 26 alphabet keys keep the flat silhouette
  //  exactly as before, which is what makes the born one stand out at
  //  all. It is a TRAILING argument so keyboard.js's existing four-arg
  //  calls are untouched.
  //
  //  When a wingMap is given the material's own colour is forced to
  //  white: three.js MULTIPLIES map by color, so leaving the hsl() tint
  //  in place would stain the whole painted texture one hue and throw
  //  away the palette the painter just built.
  function build(wingTex, color, letterTex, letterColor, opts) {
    var s = BASE;
    opts = opts || {};

    var wingMat = new THREE.MeshBasicMaterial({
      color: new THREE.Color(opts.wingMap ? '#ffffff' : color),
      map: opts.wingMap || null,
      alphaMap: wingTex || null,
    ...
    return {
      ...
      setColor: function (css) {
        ...
        //  a painted wing keeps its white multiplier -- tinting it would
        //  stain the texture and throw away its palette. The body still
        //  takes the colour, which is what ties the two together.
        if (!opts.wingMap) { wingMat.color.copy(c); }
      ...
      dispose: function () {
        ...
        //  the painted map is generated per butterfly and owned by
        //  nothing else, so it goes with the mesh. (The alphaMaps are
        //  NOT disposed here -- Wings.forDials caches and reuses those
        //  across butterflies, and freeing one would blank a live wing.)
        if (opts.wingMap) { opts.wingMap.dispose(); }
      }
    };
  }
