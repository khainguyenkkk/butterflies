# v8 — the controls say what they do, and the swarm follows your finger, 27 August 2026

Everything below is v7, unchanged. v8 adds two things.

## 1. A mark on each control

The two flowers never said what they did, and getting them the wrong way round costs a
visitor their whole name. Each now carries one: **↵** on the green, **⌫** on the red.

**Knocked out, not printed on** — drawn in the sky's own white so it reads as a hole punched
through the petals, exactly the language the wings already use for their letters. Drawn as
strokes rather than typed: there is no arrow or chevron anywhere in the piece to reuse, and
a font's own arrow glyphs are a lottery across platforms (a Quest falls back to Roboto).

A **mesh, not a sprite** — every other 2D thing here is a sprite because sprites billboard,
and that is precisely wrong for this. The flowers carry their own tilt, cant and slow sway;
a sprite ignores all three and would float free of the shape it belongs to. Parented into
the flower's own group, so the drift, the sway and the press bounce carry it for free with
no per-frame code at all.

## 2. Point, and the born butterflies come

Extend an index finger and the butterflies born from accepted names are drawn toward the
point you are aiming at, and orbit it. Let go and they disperse back to their own wide
orbits over a couple of seconds.

- **The trigger is a real gesture**, not just a tracked hand: `rig.pointing` in `hands.js`,
  index extended while the other three curl, with hysteresis. Nothing could ask this before —
  the ray is cast for any tracked hand regardless of pose, and `openness` (computed every
  tick since v2 and read by nothing) cannot tell a point from a half-open hand. Keeping it a
  deliberate gesture is what stops the swarm being dragged around while you catch letters.
- **The aim point is the first "where am I pointing" in the piece.** Until now the ray was
  only ever a scoring axis — `pickRay()` works out how far along it a target sits and throws
  that number away. It reuses v6.1's shoulder-anchored, EMA-smoothed ray rather than a fresh
  one off the fingertip; that ray exists precisely because finger curl swung a
  knuckle-anchored aim by tens of degrees, and a swarm chasing a twitching aim point would
  be unusable.
- **Distance sets how briskly each one answers**, not whether it comes at all — see the note
  in `born.js:updateAttraction()` for why the obvious version (gate the pull on proximity)
  measured at exactly zero and could never have worked.
- **Speed scales with the hand, capped at both ends**, and the orbit centre has its own hard
  speed ceiling on top.

## Verified (desktop, Chromium)

The render loop stalls when the browser pane is hidden, so these were driven by pumping the
component ticks directly with synthetic time — the same calls the loop makes.

| | |
|---|---|
| the mark tracks its flower | scale 0.96 → 1.36 peak → 1.02 settled, identical to the petals at every sample |
| the mark reads | survives to 64 px, roughly a Quest lens across the room |
| gather | five butterflies from a 3–5 m spread close to 0.88–0.97 m of the aim point (`pointGatherRadius` 0.85) |
| distance gradient | nearest (2.9 m) reaches pull 0.45 while furthest (5.5 m) reaches 0.36 |
| gather rate, tuned | mean distance remaining at 1/2/3/5 s measured across four `pointEaseMin` values — see `config.js` |
| release | pull 0.98 → 0.02 over 5 s, orbits back to 3.34–4.48 m, inside the 3.2–4.5 band |
| fast-hand sweep | peak butterfly speed 3.0 m/s (7.1 before the centre cap), cluster spread 1.14 m, no divergence, all finite |
| inert when idle | pull and centre both exactly 0 — not merely small |
| no regression | letters still catch, accept still births, loops stay bounded (born + 3) |

**Not yet run on a physical Quest.** `CFG.pointOn`/`pointOff` are the two numbers most likely
to need tuning against real hands — too low and a relaxed hand triggers the gather, too high
and a deliberate point is ignored. `rig.pointSpread` is published raw for exactly that.

## Three bugs this build turned up

- **A non-finite aim point was unrecoverable.** `camera.projectionMatrixInverse` is only
  filled during `updateProjectionMatrix()`, so before anything has rendered the mouse ray can
  hand back NaN. One NaN focus propagates into every butterfly's orbit centre and the whole
  swarm is gone for the session, silently, with nothing thrown. `pushFocus()` guards it.
- **Proximity-as-a-gate could never converge** — see above.
- **Exponential decay never arrives.** `pull` settled at 0.0057 and the orbit centre 18 mm
  off the origin forever. Invisible, but "with nobody pointing this is exactly v7's flight"
  is either true or it is not.

---

# v7 — the name becomes a butterfly, 27 August 2026

Everything below is v3 (audio) on top of v6.1, unchanged. **v7 closes the generation stage
that every version since v3 left open**: accepting a name now births a butterfly out of it.

- It **bursts from where the name hangs**, accelerating outward with its wings beating hard,
  then eases into a wide orbit at 3.2–4.5 m — clear of the swarm's own 1.0–2.4 m, so it is
  never mistaken for a letter and never flies through the keyboard.
- It **carries the name** as a label (`UI.wordmark`, exported since v4 and never once called
  until now), hung off a position-only anchor and scaled angularly so it stays readable out
  at four metres.
- Its **wing shape is derived from the name** — `born.js:nameToDials()` answers the open
  question `CLAUDE.md` has carried since v3. Deterministic: the same name is the same
  butterfly, on any machine, in any session.
- Its **wing is painted**, not flat — warped fBm posterised into bands, plus venation,
  marginal chevrons, eyespots and scale shimmer (`js/wing-paint.js`). The 26 letter
  butterflies stay flat and solid, which is what makes this one stand out at all. This is
  the only place the piece's no-gradients rule is suspended.
- It **carries one of the three butterfly sounds** at random, spatialised the same way the
  swarm's three are, with its own falloff band for the wider orbit.

Capped at `CFG.bornMax` (12); the oldest fades out and is fully disposed, audio included.

New files: `js/noise.js`, `js/wing-paint.js`, `js/born.js`. Changed: `app.js` (the seam),
`bfly-model.js` (optional colour map), `audio.js` (`detachLoop` + per-loop falloff),
`config.js`, `index.html`. Nothing about the keyboard, the flight, the typography or the
capture flow changed.

## Verified (desktop, Chromium)

| | |
|---|---|
| catch letters, press accept | butterfly born, bursts from the name, reaches orbit |
| determinism | same name → identical dials and pixel-identical wing; different names differ |
| all 12 orbits | radius 3.5–4.31 m, every one outside the swarm's 2.4 m edge |
| the cap | 20 spawned in one frame → settles at exactly 12, no throw |
| audio | born loop plays and pans; loops return to 3 after teardown — no leak |
| scene weight | +4 renderables per butterfly (3 meshes, 1 sprite); 144 → 180 meshes at 12 |
| accept-frame cost | 3.3 ms — the paint is deferred one frame, see `CLAUDE.md` |
| wing paint | ~99 ms @ 256, 76 @ 192, 23 @ 128 — one-time, off the accept frame |
| console | clean, including under rapid-fire spawn past the cap |

**Not yet run on a physical Quest.** The paint cost is the thing to watch there — it is
several times a desktop's, and `CFG.bornTexSize` is the one-number lever if it bites.
