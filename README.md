# v9.3 — the greeting holds the room, and a held hand attracts

A fourth pass. Everything in the v9.2, v9.1 and v9 notes below still stands.

---

## 1. Nothing is interactive while the greeting speaks

For `welcomeLockSec` (17 s) after a reset — the span the voice is talking over — **every**
butterfly interaction is switched off: picking, hover, pinch, pointing, holding, the wave,
and the desktop letter-key shortcuts.

This is not politeness, it is how the piece is actually used. Someone who has just put the
headset on has their hands up somewhere, and any of those hands landing on a butterfly during
the welcome means they start their turn with letters already caught, or the swarm already
dragged out of shape, before they have been told what any of it does.

**A clock, not "has the audio finished".** The voice is *scheduled* the moment SFX is ready
but stays silent until the AudioContext is resumed by a user gesture (audio.js's autoplay
note), so the clip can end far later than the text does — or never, on a machine where that
gesture never arrives. A timer always ends, so the piece can never be left permanently
uninteractive by an audio quirk. 17 s slightly outlasts `welcomeSpeechSec` (16.5) so the lock
releases after the last word rather than under it.

`interact.js` returns on **one branch at the top of `tick()`** rather than guarding each
gesture — it cannot be partially applied — and clears the focus/hold lists and any latched
pinch or click first, so the frame the lock lifts does not fire whatever was pressed during
it.

Verified while locked: a click driven through the real `pointer-input` path leaves `typed`
empty, the letter-key shortcut does nothing, and a 2.6 m/s hand produces **0** gusts. After
the lock lifts, the same click types `A` and the same hand produces 1 gust.

---

## 2. A new gesture: hold, and your butterfly comes to your hand

Close a hand and keep it closed, and the butterfly born from your name flies to it and
circles slowly, close in.

**The gesture** — `hands.js:holding`. `openness` (0 = fist, 1 = splayed palm) has existed
since v2 with nothing reading it for this. Two thresholds, like the pinch and the point, or it
chatters.

**The dwell is the part that matters, and neither other gesture needs one.** Catching a letter
closes the hand as well — a pinch *is* a partial fist — so without a dwell every capture would
read as the start of a hold, and the butterfly you just caught would be chased by the one you
made earlier. `holdDwell` (0.55 s) separates them cleanly without feeling like a wait. A
pointing hand can never also be holding: three curled fingers drag `openness` below the
threshold, and the two would otherwise fight over the same butterfly.

Verified by exercising the exact gesture logic: a 0.3 s pinch **never** latches; a sustained
hold latches at **0.53 s**; a pointing hand never latches; and hysteresis keeps an established
hold alive at openness 0.38 (between the 0.30 on and 0.45 off thresholds).

**The attraction** reuses the same orbit-centre machinery as pointing — which is why "the
butterfly follows my hand" falls out of the flight that already exists rather than needing a
second motion model. Only the constants differ, and they differ a lot:

| | point | hold |
|---|---|---|
| gather radius | 1.70 m (a loose surround) | **0.30 m** (circling your hand) |
| max pull | 0.55 (leans, never commits) | **1.0** (a butterfly on your hand is on your hand) |
| orbit rate | 0.25 rad/s | **0.28** rad/s, but around a 0.30 m circle — a hover |
| spring | k 9.0, damp 0.72 | **k 3.4, damp 0.42** — slower approach, bigger overshoot |
| centre speed cap | 0.90 m/s | **0.75 m/s** |

A hold **beats** a point outright rather than blending — they would otherwise pull the same
butterfly to two places at once — and `heldAmt` eases, so switching between them is a glide.

Measured, from a butterfly 1.44 m away: the centre reaches the hand in about 2 s, peaks at
exactly the 0.75 m/s cap, **overshoots and curves back**, and the butterfly then hovers at
**0.300 m** — exactly `holdRadius`. On release, pull decays to **exactly 0** and the centre
returns to **exactly** the origin by ~12 s.

One thing that had to be bypassed: `orbitAt`'s radius has a hard floor of 1.2 m, so a
butterfly circling your hand at 30 cm could never get there. The floor applies to the orbit's
own radius; the *final* radius is lerped past it, leaving the flight identical everywhere
else.

The hold point is the **wrist**, not the fingertip: a closed hand's fingertips are curled into
the palm and wander as the fist tightens, while the wrist is the steadiest joint on a closed
hand and sits about where anyone would say their hand is.

---

## 3. A fourth card — "Holding hand to attract your butterfly."

Chained after the waving card, docking into slot 2. Verified: point → wave → hold arrive in
order into slots 0, 1, 2, with no two ever big and centred at once.

### The docked cards were clipping, and the fix is aspect-proof

The cards were anchored by their **centre** at a fraction of the half-width, which leaves the
outer half-card hanging past it — fine on a wide view, and clipped straight off the screen on
a narrow one. Found in a narrow preview pane, and a Quest's aspect is not a desktop's either.

`dockOffset` now positions the card's outer **edge** and insets the centre by half the card.
Verified at a punishing 280 px pane width: all three cards' top-right corners land at exactly
NDC (0.94, …) and every card is fully on screen with its text complete.

---

## Files changed in v9.3

| file | change |
|---|---|
| `js/hands.js` | `holding`/`holdT`, the dwell + hysteresis; `tick()` now reads `dtMs` |
| `js/interact.js` | the lock branch at the top of `tick()`; publishes `holds` |
| `js/born.js` | `heldAmt`, the hold attractor and its spring, hold constants in `orbitAt` |
| `js/welcome.js` | `Welcome.locked()` and `_lockUntil`, set on every greeting |
| `js/keyboard.js` | letter-key shortcuts silent while locked |
| `js/wave.js` | no gusts while locked |
| `js/tutorial.js` | the `hold` card, chained after `wave`; edge-anchored docking |
| `js/config.js` | `welcomeLockSec`, `hold*` gesture and attraction blocks, dock fractions |
